...
class users_Model_DbTable_DbAcl extends Zend_Db_Table_Abstract
{
	protected  $_name = "tbwu_acl_acl";
	//get infomation of acl
	public function getAclInfo($sql)
	{
		$db = users_Model_DbTable_DbAcl::getAdapter();  		
  		$stm = $db->query($sql);
  		$row = $stm->fetchAll();
  		if(!$row) return NULL;
  		return $row;
...
//     	$datamodule = array(
//     		"user_type_ida" = $arr["user_type_id"];
//     	);
//     	return $this->insert("rsv_acl_acl",$datamodule);
	}
	...
	public function updateStatus($arr)
	{
		...
		$where = $this->getAdapter()->quoteInto('acl_id=?', $arr["unstatus"]);
	...
	public function updateUnStatus($arr)
	{
		...
		$where = $this->getAdapter()->quoteInto('acl_id=?', $arr["status"]);
		$this->update($array_data, $where);
			
	}
	public function deleteAcl($id){
		$db = $this->getAdapter();
		$sql = "Delete From tbwu_acl_acl Where acl_id=".$id;
		$row = $db->query($sql);
		return $row;
	}		
}
