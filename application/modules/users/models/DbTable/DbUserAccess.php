...
class users_Model_DbTable_DbUserAccess extends Zend_Db_Table_Abstract
{
	protected  $_name = "tbwu_acl_user_access";
	
	public function getUserAccess($id)
	{
		$db=users_Model_DbTable_DbUserAccess::getAdapter();  
	...
	public function insertUserAccess($arr,$id)
	{
		if(empty($arr["user_access"])){
			...
			Application_Form_FrmMessage::redirector("/userWu/user-access/edit/id/".$id);exit();
			//break;
		}else
// 		$data=array(
// 			"user_type_id" => $id,
